...
- `deferred_bundle` (CUUN_TAGS): same as `TagsCombinator`'s `deferred_bundle` — any axis that was a `TagsRandomPick`/`TagsRandomBundle` output (serialized as-is by `Rules to JSON`, never expanded) round-trips here and resolves independently per combination. Wire into a separate `TagsMerge.bundle_i` slot.
